...
inline uint32_t getMaterialIDFromPosition(const Vec3& centroid) {
    ...
    // Floor, ceiling, and back wall are white room surfaces
    if (centroid.y < -0.49f || centroid.y > 0.49f || centroid.z < -0.49f) {
        return MAT_WHITE;
    }
    
    // Remaining triangles are on one of the two inner boxes.
    // Assign to the nearest box center (XZ distance).
    float dxShort = centroid.x - kShortBoxCenterX;
    float dzShort = centroid.z - kShortBoxCenterZ;
    float dxTall  = centroid.x - kTallBoxCenterX;
    float dzTall  = centroid.z - kTallBoxCenterZ;
    float distShortSq = dxShort * dxShort + dzShort * dzShort;
    float distTallSq  = dxTall  * dxTall  + dzTall  * dzTall;
    return (distShortSq < distTallSq) ? MAT_SHORT_BOX : MAT_TALL_BOX;
}
