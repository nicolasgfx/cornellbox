...
int main(int argc, char** argv) {
    ...
    // 8) Export OBJ + MTL with material groups (for external renderers).
    const std::string mtlFile = config.outputPath + "/cornellbox.mtl";
    const std::string matObjFile = config.outputPath + "/cornellbox.obj";
    OBJExporter::exportMTL(mtlFile, mesh);
    OBJExporter::exportOBJWithMaterials(matObjFile, "cornellbox.mtl", mesh);

    // 9) Ray-traced PNG renders (OptiX).
#ifdef USE_OPTIX
    {
